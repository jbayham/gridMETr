...
#2018 cbsa
cbsa2 <- read_sf("data/tl_2018_us_cbsa/tl_2018_us_cbsa.shp") %>% 
...
poptotals <- pop %>% 
  group_by(cbsa) %>% 
  summarise(poptotal = sum(popden2015, na.rm = T)) %>% 
  ungroup() %>% 
...
write_rds(poptotals,"/RSTOR/pierce_pm/cbsa3_popweights.rds")
...
for(j in 1:length(outer.shell)){
  ...
  background.final <- left_join(bg,bgfill,by=c("lon","lat","Month")) %>% 
    ...
    group_by(cbsa,date) %>%
    summarise(weightedpm25 = sum(popweight*pm25, na.rm = T),
              weightedpmbnew = sum(popweight*PMb, na.rm = T),
              weightedpmover = sum(popweight*PMover, na.rm = T),
              weightedhms = sum(popweight*hms, na.rm = T)) %>%
    ungroup()

  write_rds(background.final,paste0("data/pierce_bg/census2018_",paste0(2006+j-1),".rds"))
  
}

fl <- str_subset(list.files("data/pierce_bg", full.names = T),"census2018")
...
write_rds(alltogether,"../../../../RSTOR/pierce_pm/weighteddata2_census2018.rds")
